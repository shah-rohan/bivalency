...
violin_plot = function(gene_expression_df, filter_set = consensusGenes, title_text)
{
  violin_df = gene_expression_df %>% filter(refseq_id %in% filter_set) %>% select(refseq_id:npc) %>% mutate(across(.cols = naive:npc, .fns = ~log10(.+.0001))) %>% gather(key = "cell_type", value = "log10_tpm", -refseq_id)
violin_df$cell_type = factor(violin_df$cell_type, levels = c('naive', 'primed', 'npc'), ordered = TRUE)

violin_plt = ggplot(data = violin_df, aes(x = cell_type, y = log10_tpm, fill = cell_type)) + geom_violin() +  stat_summary(fun=mean, geom="point", shape=23, size=2, fill = "black") + theme_classic() + geom_boxplot(fill = "white", width = 0.1) + scale_fill_manual(values = c("firebrick2", "royalblue2", "green3")) + labs(title = title_text)
print(violin_plt)
}
...
gene_expression_bivalent = gene_expression %>% filter(refseq_id %in% bivalent_genes_25$naive)
violin_df = gene_expression_bivalent %>% select(refseq_id:npc) %>% mutate(across(.cols = naive:npc, .fns = ~log10(.+.0001))) %>% filter(naive>=-3) %>% gather(key = "cell_type", value = "log10_tpm", -refseq_id)
violin_df$cell_type = factor(violin_df$cell_type, levels = c('naive', 'primed', 'npc'), ordered = TRUE)
...
for (i in 1:5)
{
  gene_expression_bivalent = gene_expression %>% filter(refseq_id %in% bivalent_genes_25$naive)
  countup = floor(length(gene_expression_bivalent$naive)/5)
  violin_df = gene_expression_bivalent %>% arrange(desc(npc)) %>%  slice((countup*(i-1)):(countup*i - 1)) %>% select(refseq_id:npc) %>% mutate(across(.cols = naive:npc, .fns = ~log10(.+.0001))) %>% gather(key = "cell_type", value = "log10_tpm", -refseq_id)
  violin_df$cell_type = factor(violin_df$cell_type, levels = c('naive', 'primed', 'npc'), ordered = TRUE)
...
print(paste0("Proportion of mid dom bivalent genes that are DE: ", 100-100*(nonregulated_genes$naive_npc %in% non_biv %>% sum)/(non_biv %in% commonGenes %>% sum)))
...
hmd_diffexp_naive_npc = hmds$naive %>% inner_join(sleuth_table_gene$naive_npc, by = c("Gene" = "refseq_id")) %>% select(Gene:k27, trans, qval) %>% mutate(diffexp = ifelse(qval <= 0.05, 1, 0)) %>% mutate(diffexp = as.factor(diffexp)) %>% mutate(logk4 = log(k4+1), logk27 = log(k27+1), logtrans = log(trans+1), logk9 = log(k9+1)) %>% select(diffexp, k4, logk4, k27, logk27, k9, trans, logtrans)
...
On the whole, we find that the model with K4 and K27 with logs of both minimizes BIC by 18.21, which is massive.
...
hmd_diffexp_primed_npc = hmds$primed %>% inner_join(sleuth_table_gene$primed_npc, by = c("Gene" = "refseq_id")) %>% select(Gene:k27, trans, qval) %>% mutate(diffexp = ifelse(qval <= 0.05, 1, 0)) %>% mutate(diffexp = as.factor(diffexp)) %>% mutate(logk4 = log(k4+1), logk27 = log(k27+1), logtrans = log(trans+1)) %>% select(diffexp, k4, logk4, k27, logk27, k9, trans, logtrans)
...
```{r overlap_ouranalysis}

ouranalysis = list()
ouranalysis$mikkelsen = read.table("full_genes/Bivalent_refseq_Mikkelsen.bed", stringsAsFactors = F, header = F)$V4
ouranalysis$mas = read.table("full_genes/Bivalent_refseq_Mas.bed", stringsAsFactors = F, header = F)$V4
ouranalysis$xiao = read.table("full_genes/Bivalent_refseq_Xiao.bed", stringsAsFactors = F, header = F)$V4
...
for (i in names(ouranalysis))
{
  ...
  metagene_plot(metagene(naive, tss_range, ouranalysis[[i]]), title = paste0("Naive cells at bivalent genes from ouranalysis analysis of ", i))
  metagene_plot(metagene(primed, tss_range, ouranalysis[[i]]), title = paste0("Primed cells at bivalent genes from ouranalysis analysis of ", i))
  metagene_plot(metagene(npc, tss_range, ouranalysis[[i]]), title = paste0("NPC cells at bivalent genes from ouranalysis analysis of ", i))
  
  naive_not_i = setdiff(bivalent_genes_25$naive[bivalent_genes_25$naive %in% consensusGenes], ouranalysis[[i]])
  metagene_plot(metagene(naive, tss_range, naive_not_i), title = paste0("Naive cells at naive bivalent genes NOT from ouranalysis analysis of ", i))
  metagene_plot(metagene(primed, tss_range, naive_not_i), title = paste0("Primed cells at naive bivalent genes NOT from ouranalysis analysis of ", i))
  metagene_plot(metagene(npc, tss_range, naive_not_i), title = paste0("NPC cells at naive bivalent genes NOT from ouranalysis analysis of ", i))
...
simple_roc = function(labels, hmds, rev = F)
{
  ...
  roc_plt = ggplot(data = roc_df, aes(x = fpr, y = tpr, color = hmd)) + geom_line(size=1) + scale_color_gradientn(colors = pal, limits = c(0, 100)) + theme_classic()
  print(roc_plt)
...
simple_roc_logs = function(labels, hmds, rev = F)
{
  df = data.frame(labs = labels, hmd = hmds)
  ...
  roc_plt = ggplot(data = roc_df, aes(x = fpr, y = tpr, color = hmd)) + geom_line(size=1) + scale_color_gradientn(colors = pal) + theme_classic()
  print(roc_plt)
...
TODO: Add ROCs for all the mods and for the different linear models.