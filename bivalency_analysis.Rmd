...
```{r gene_ontology_functions}

testGO = function(GOdata){
  classic_fisher_result = runTest(GOdata, algorithm = "classic", statistic = "fisher")
  allGO = usedGO(GOdata)
  all_res = GenTable(GOdata, classicFisher = classic_fisher_result, orderBy = 'classicFisher', topNodes = length(allGO))
...
We will now conduct GO analysis on the datasets described above.

```{r gene_ontology_dominance_runner}

go_tables = list()
go_tables$k27 = testGO(godata$k27)
go_tables$k4 = testGO(godata$k4)
go_tables$mid = testGO(godata$mid)
