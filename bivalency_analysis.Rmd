...
Having the above gene expression matrix, I want to examine the way in which gene expression changes across these different datasets from naive to NPC at naive bivalent genes.
...
violin_plt = ggplot(data = violin_df, aes(x = cell_type, y = log10_tpm, fill = cell_type)) + geom_violin() +  stat_summary(fun=mean, geom="point", shape=23, size=2, fill = "black") + theme_classic() + geom_boxplot(fill = "white", width = 0.1) + scale_fill_manual(values = c("firebrick2", "royalblue2", "green3")) + theme(text = element_text(family = "Arial"))
print(violin_plt)
