...
hmd_diffexp = hmds$naive %>% inner_join(sleuth_table_gene$naive_npc, by = c("Gene" = "refseq_id")) %>% select(Gene:logratio, qval) %>% mutate(diffexp = ifelse(qval <= 0.05, 1, 0)) %>% mutate(diffexp = as.factor(diffexp)) %>% mutate(logk4 = log(k4+1), logk27 = log(k27+1))

# Trivial Model
summary(glm(diffexp ~ 1, data = hmd_diffexp, family = binomial))$aic

# K4 only
summary(glm(diffexp ~ k4, data = hmd_diffexp, family = binomial))$aic

# K27 only
summary(glm(diffexp ~ k27, data = hmd_diffexp, family = binomial))$aic

# K9 only
summary(glm(diffexp ~ k9, data = hmd_diffexp, family = binomial))$aic

# trans only
summary(glm(diffexp ~ trans, data = hmd_diffexp, family = binomial))$aic

# K4 + K27
summary(glm(diffexp ~ k4 + k27, data = hmd_diffexp, family = binomial))$aic

# K4 + K27 + trans
summary(glm(diffexp ~ k4 + k27 + trans, data = hmd_diffexp, family = binomial))$aic

# K4 + K27 + log of each
summary(glm(diffexp ~ k4 + k27 + logk4 + logk27, data = hmd_diffexp, family = binomial))$aic

# K4 + K27 + log of each + bivalency
summary(glm(diffexp ~ k4 + k27 + logk4 + logk27 + trans, data = hmd_diffexp, family = binomial))$aic

```

On the whole, we find that the model with K4 and K27 with logs of both minimizes AIC by 1.9, meaning that it is roughly 2.6x as likely to be the best model over the model with trans bivalency.

We will now show the logistic regression therein.

```{r logistic_regressions}

glm_k4_k27_logs = glm(diffexp ~ k4 + k27 + logk4 + logk27, data = hmd_diffexp, family = binomial)

hmd_diffexp$predicted = predict(glm_k4_k27_logs, newdata = hmd_diffexp, type = "response")
plot(x = hmd_diffexp$predicted, y = hmd_diffexp$diffexp)

glm_k4_k27_logs_trans = glm(diffexp ~ k4 + k27 + logk4 + logk27 + trans, data = hmd_diffexp, family = binomial)

hmd_diffexp$predicted_trans = predict(glm_k4_k27_logs_trans, newdata = hmd_diffexp, type = "response")
plot(x = hmd_diffexp$predicted_trans, y = hmd_diffexp$diffexp)
...
summary(glm_k4_k27_logs)

# K4, K27, log K4, log K27, trans
summary(glm_k4_k27_logs_trans)
...
sample = sample.int(n = nrow(hmd_diffexp), size = floor(.7*nrow(hmd_diffexp)), replace = F)

hmd_diffexp_train = hmd_diffexp[sample,]
hmd_diffexp_test = hmd_diffexp[-sample,]

glm_k4_k27_logs_train = glm(diffexp ~ k4 + k27 + logk4 + logk27, data = hmd_diffexp_train, family = binomial)
glm_k4_k27_logs_trans_train = glm(diffexp ~ k4 + k27 + logk4 + logk27 + trans, data = hmd_diffexp_train, family = binomial)
glm_k4_k27_logs_k9_train = glm(diffexp ~ k4 + k27 + logk4 + logk27 + k9, data = hmd_diffexp_train, family = binomial)

hmd_diffexp_test$predicted = predict(glm_k4_k27_logs_train, newdata = hmd_diffexp_test, type = "response")
hmd_diffexp_test$predicted_trans = predict(glm_k4_k27_logs_trans_train, newdata = hmd_diffexp_test, type = "response")
hmd_diffexp_test$predicted_k9 = predict(glm_k4_k27_logs_k9_train, newdata = hmd_diffexp_test, type = "response")

plot(x = hmd_diffexp_test$predicted, y = hmd_diffexp_test$diffexp)
plot(x = hmd_diffexp_test$predicted_trans, y = hmd_diffexp_test$diffexp)
plot(x = hmd_diffexp_test$predicted_k9, y = hmd_diffexp_test$diffexp)

hmd_diffexp_test = hmd_diffexp_test %>% mutate(predicted_dir = ifelse(predicted>0.5, 1, 0), predicted_dir_trans = ifelse(predicted_trans>0.5, 1, 0), predicted_dir_k9 = ifelse(predicted_k9>0.5, 1, 0))

mean(1==hmd_diffexp_test$diffexp)
mean(hmd_diffexp_test$predicted_dir==hmd_diffexp_test$diffexp)
mean(hmd_diffexp_test$predicted_dir_trans==hmd_diffexp_test$diffexp)
mean(hmd_diffexp_test$predicted_dir_k9==hmd_diffexp_test$diffexp)
